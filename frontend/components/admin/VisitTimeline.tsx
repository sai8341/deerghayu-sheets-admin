                        ...
                        <div className="bg-white rounded-xl p-0 border border-gray-200 shadow-sm hover:shadow-md hover:border-ayur-300 transition-all overflow-hidden">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-gray-50 border-b border-gray-100">
                                <div className="flex items-center gap-3">
                                    ...
                                    {visit.status && (
                                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border ${visit.status === 'completed' ? 'bg-green-100 text-green-700 border-green-200' :
                                                visit.status === 'in_progress' ? 'bg-blue-100 text-blue-700 border-blue-200' :
                                                    'bg-amber-100 text-amber-700 border-amber-200'
                                            }`}>
                                            {visit.status === 'booked' ? 'Booked' : visit.status === 'in_progress' ? 'In Progress' : 'Completed'}
                                        </span>
                                    )}
                                </div>
                                <Button
                                ...
                                >
                                    {visit.status === 'booked' ? 'Start Consultation' : 'View Details'} <ChevronRight size={14} className="ml-1" />
                                </Button>
                            </div>
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <p className="text-sm font-medium text-gray-900 leading-relaxed bg-gray-50/50 p-2 rounded border border-gray-100/50">
                                            {visit.diagnosis || <span className="text-gray-400 italic">Pending...</span>}
                                        </p>
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <p className="text-sm text-gray-800 leading-relaxed">
                                            {visit.treatmentPlan || <span className="text-gray-400 italic">Pending...</span>}
                                        </p>
                                    </div>
                                </div>
