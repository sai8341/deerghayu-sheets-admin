        ...
        if (e.target.files && e.target.files.length > 0) {
            ...
            for (const file of newFiles) {
                ...
                if (file.size > 5 * 1024 * 1024) {
                    addToast(`Skipped ${file.name}: Too large (>5MB).`, 'error');
                    continue;
                }
                ...
                <div>
                    ...
                    {formData.attachmentFiles.length > 0 && (
                        <div className="mt-2 space-y-1">
                            {formData.attachmentFiles.map((f, i) => (
                                <div key={i} className="flex justify-between items-center text-xs bg-gray-50 p-2 rounded">
                                    <span className="text-gray-700 truncate">{f.name}</span>
                                    <button type="button" onClick={() => removeFile(i)} className="text-red-500 hover:text-red-700 ml-2">Remove</button>
                                </div>
                            ))}
                        </div>
                    )}
